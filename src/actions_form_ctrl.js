  ...
  async function callback () {
    ...
    if (rowData.order_state) {
      if (rowData.order_state.toLowerCase() === cons.STATE_PLAN.toLowerCase()) {
        utils.alert('warning', 'Warning', 'This order has NOT been released')
        return
      }
      if (rowData.order_state.toLowerCase() === cons.STATE_CLOSE.toLowerCase()) {
        utils.alert('warning', 'Warning', 'This order has been closed')
        return
      }
    }
...
function getRowData (callback, tags) {
  ...
  utils.get(url)
    ...
    .catch((e) => {
      utils.alert('error', 'Database Error', 'Database connection failed with influxdb')
      console.log(e)
    })
}
...
function showAlerts (result, id, state) {
  if (result.ok) {
    utils.alert('success', 'Success', `Order ${id} has been marked as ${state}`)
    closeForm()
    tableCtrl.refresh()
  } else {
    utils.alert(
